...
public class AmapNaviReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (keyType == 12110) {
            // 区间测速
            manager.resetWatchdog();
            handleIntervalSpeed(intent, manager);
            return;
        }
    ...
    private void handleIntervalSpeed(Intent intent, FloatingWindowManager manager) {
        int averageSpeed = intent.getIntExtra("AVERAGE_SPEED", 0);
        int limitedSpeed = intent.getIntExtra("LIMITED_SPEED", 0);
        String endDistanceText = intent.getStringExtra("END_DISTANCE_TEXT");
        if (endDistanceText == null) endDistanceText = "";
        // 区间起点距离：
        //  - KEY_TYPE=12110 且 INTERVAL_DISTANCE == 0.0 时，START_DISTANCE 即「距起点」→ 显示胶囊
        //  - KEY_TYPE=12110 但 INTERVAL_DISTANCE != 0.0（已过起点）→ 隐藏胶囊
        double intervalDistance = readDistance(intent, "INTERVAL_DISTANCE");
        double startDistance = readDistance(intent, "START_DISTANCE");
        // 先刷新区间测速主体，最后再写起点距离，确保起点距离是最后落定值、不被覆盖
        manager.updateIntervalSpeed(averageSpeed, limitedSpeed, endDistanceText);
        if (intervalDistance == 0.0 && startDistance >= 0) {
            manager.updateIntervalStartDistance((int) Math.round(startDistance));
        } else {
            manager.updateIntervalStartDistance(-1); // 隐藏：已过起点 / 数据无效
        }
        txInterval(averageSpeed, limitedSpeed, endDistanceText);
    }

    /**
     * 兼容读取距离字段：高德可能用 int 也可能用 double 发送，
     * 直接用 getDoubleExtra 读 int 类型会拿不到值（返回默认 -1.0），这里兼容两种类型。
     */
    private double readDistance(Intent intent, String key) {
    ...
    private void txInterval(int avgSpeed, int limitedSpeed, String endDistanceText) {
        try {
            JSONObject j = new JSONObject();
            j.put("t", 11);
            j.put("avgSpeed", avgSpeed);
            j.put("limitedSpeed", limitedSpeed);
            j.put("endDistanceText", endDistanceText);
            tx(j);
